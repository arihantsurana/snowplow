...
object IabEnrichmentSpiderCfLineSpec {
 ...
 * to an event.
 */
class IabEnrichmentSpiderCfLineSpec extends Specification with EnrichJobSpec {
  import EnrichJobSpec._
  ...
  "A job which processes a CloudFront file containing 1 valid custom unstructured event" should {
    runEnrichJob(IabEnrichmentSpiderCfLineSpec.lines, "cloudfront", "1", false, List("geo", "domain"), iabEnrichmentEnabled = true)

    "correctly attach a derived context fetched from the IAB enrichment" in {
      val Some(goods) = readPartFile(dirs.output)
      goods.size must_== 1
      val actual = goods.head.split("\t").map(s => if (s.isEmpty()) null else s)
      for (idx <- IabEnrichmentSpiderCfLineSpec.expected.indices) {
        actual(idx) must BeFieldEqualTo(IabEnrichmentSpiderCfLineSpec.expected(idx), idx)
      }
    }
